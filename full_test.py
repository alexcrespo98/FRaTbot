...
def main():
    ...
    # --- User prompts ---
    waveform_val = float(read_input("Waveform fraction (motion vs stop, 0-1) [0.5]: ", "0.5", float))
    if waveform_val < 0.0 or waveform_val > 1.0:
        print("Invalid waveform value, using default 0.5")
        waveform_val = 0.5
    ...
    # Configure ESP: Ivan, interval, waveform
    ser.write(f"ivan:{1 if use_ivan else 0}\n".encode())
    ser.write(f"interval:{interval_ms}\n".encode())
    ser.write(f"waveform:{waveform_val}\n".encode())
    time.sleep(0.05)
